...
Implementation risk is managed by building the pipeline in phases, position, then height, then multi-person clustering, then speaker detection, each on an already-working previous phase.
mmWave is the most expensive candidate reviewed, though, so mentor approval of that cost is a precondition.
Sensor fusion (mmWave or 2D LiDAR with thermal) wasn't adopted as the starting point.
mmWave alone already has a plausible path to height, no fusion combination is tested together in the literature, and thermal needs two units to match mmWave's coverage.
Fusion stays a documented fallback, not a precondition for starting.
