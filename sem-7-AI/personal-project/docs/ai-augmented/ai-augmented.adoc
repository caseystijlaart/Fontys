...
* Claude by Anthropic
** Used for brainstorming, verification, and critique of ideas and concepts.
** Image related tasks, such as generating images for the startup screen of the app and creating a logo for the project, as it was able to create such things much better than ChatGPT Codex, which struggled with image related tasks and often produced subpar results.

* Claude Code by Anthropic
** Used for code review and debugging, similar to ChatGPT Codex, but with a different approach and capabilities.
** Main usage was for an flutter front-end IOS app, as it provided better support for and understanding when it came to my vision for the app.
...
=== Claude by Anthropic : Brainstorming, ideation, and Image Generation

==== Image Generation
For the application I wanted to make use of an start up screen, which I wanted to generate using AI, as I wanted it to be visually appealing and to fit the overall aesthetic of the app.

[quote,Claude Code Image Generation]
"Generate an gif/video that i can use as my start up screen for a plant health monitoring system. Keep it simple but relevant."

I initially tried to use ChatGPT Codex for this task, but it struggled with generating a high-quality startup screen and often produced subpar results that did not meet my expectations. See figure <<ChatGPT-image>> for an example of the generated startup screen from ChatGPT, which was quite overwhelming and did not fit the overall aesthetic of the app.

[[ChatGPT-image]]
.ChatGPT Image Generation
image::chatGPT-image.png[width=450]

However, when I tried using Claude Code by Anthropic for the same task, it was able to generate a much better startup screen that was visually appealing and fit the overall aesthetic of the app. See figure <<claude-image>> for an example of the generated startup screen from Claude Code, which was much more in line with my vision for the app and provided a much better user experience.

[[claude-image]]
.Claude Image Generation
image::claude-image.png[width=450]

After the initial success with the startup screen, I went back and forth with Claude to improve the design, which went quite well, as it was able to understand my feedback and make the necessary adjustments to achieve the desired outcome. 
This is shown in the figure <<claude-image-rev>>, where the design is more in line with my original vision for the app, and it provided a much better user experience compared to the initial design generated by ChatGPT.

[[claude-image-rev]]
.Claude Image Generation (Revised)
image::claude-image-rev.png[width=450]

<<<

=== Claude Code by Anthropic : Code Review and Debugging, Image Generation

==== Front End Development
As I was working on the software development aspect of my project, I had to write a lot of code for the front-end of the app, which was built using Flutter and had no prior experience with it.
While I initially used ChatGPT Codex for code review and debugging, I found that Claude Code by Anthropic provided better support and understanding when it came to my vision for the app, especially for the front-end development using Flutter.
I mainly switched to claude once I realized that ChatGPT Codex was struggling to understand my vision for the app and was not providing the necessary support for the front-end development, which was a crucial aspect of the project.
And besides that, as it had already proven itself to be better at image generation, I thought it would also be better for the front-end development, as it would have a better understanding of the visual and design aspects of the app.

To provide some examples of how Claude Code was used for the front-end development, see figure <<claude-frontend-1>> and <<claude-frontend-2>>, where it provides e.g. issues/bugs that appeared upon build and needed help with to fix them.

[[claude-frontend-1]]
.Claude Code Front-end Development : Overflow Issue
image::claude-overflow.png[width=450]

[[claude-frontend-2]]
.Claude Code Front-end Development : Unfit Graph
image::claude-graph-unfit.png[width=450]

==== Debugging 
When it came to debugging, I found that Claude Code by Anthropic was also quite helpful, as it provided more accurate and relevant suggestions for fixing issues in the code. 
For example, when I was working on the front-end development using Flutter, I encountered an issue where the app was crashing when I tried implementing a specific feature.
Claude Code had access to my github repository and I had provided the given error logs, so it was able to analyze the code and the error logs to identify the root cause of the issue and provide a specific solution to fix it.
As someone who does not have a lot of experience with Flutter, this was quite helpful, as it allowed me to quickly resolve the issue and continue with the development of the app without needing to spend a lot of time troubleshooting and debugging on my own. See figure <<claude-debugging-1>> and <<claude-debugging-2>> for examples of the debugging process with Claude Code, where it provided specific suggestions for fixing the issue in the code.

[[claude-debugging-1]]
.Claude Code Debugging 1
image::claude-debug.png[]

[[claude-debugging-2]]
.Claude Code Debugging 2
image::claude-debug-2.png[]

==== Code Review
When it came to code review, I ended up using Claude Code and the general chat function more as it provided with more insightful answers and suggestions for improving the code.

As I wanted to validated whether my ESP32 was making somewhat sense when it came to predicting when the plant had to be watered, I asked Claude to with the current plant metrics, soil type and plant type, and based upon that, predict when the plant would need to be watered, and then I compared that to the actual watering prediction system I had implemented on the ESP32. 
I noticed some discrepancies between the two, which made me question whether my implementation on the ESP32 was correct or not.
As I personally knew from the experience with my plants, Claude's predictions were more in line with my expectations, which made me question the accuracy of my implementation on the ESP32.
I then asked Claude to review my code and provide feedback on my trained model and the overall implementation of the watering prediction system on the ESP32, and it provided me with specific suggestions for improving the code and the model to achieve better accuracy in the predictions.

[[claude-predict]]
.Claude Code Review and Prediction
image::claude-predict.png[]

[[claude-rev-cap]]
.Claude Code Review and Prediction (Revised)
image::claude-rev-cap.png[]

An example of how Claude was used to improve my implementation on the ESP32 can be seen in the figures <<claude-rev-1>> and <<claude-rev-2>>, where it provided specific suggestions for improving the code and the model to achieve better accuracy in the predictions, which ultimately led to a more accurate watering prediction system on the ESP32.

[[claude-rev-1]]
.Claude Code Review and Improvement 1
image::claude-rev-1.png[]

[[claude-rev-2]]
.Claude Code Review and Improvement 2
image::claude-rev-2.png[]